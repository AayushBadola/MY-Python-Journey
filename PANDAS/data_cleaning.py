...
#### IMPORTANT WE CAN REMOVE THE ENTIRE ROW IF A VALUE IN ROW 1 AND ROW 2 say column =  NAME MATCHES 

#### so if there is a row 
'''
       No        Name    Type1   Type2  Height  Weight  Legendary
0      1   Bulbasaur    Grass  Poison     0.7     6.9          0
129    3   Bulbasaur    Poison Water      0.5     3            1

'''

# and we want to remove ANY ROW WHOSE NAME is same (doesnt matter if later columns are different) as long as name1 = name2 we remove the later
# BY DEFAULT THE LATER ONE IS REMOVED 
# IF WE DO df.drop_duplicate(subset=  ["Name"]) # it rmoves the entire rows in which the names are same
# if we do df.drop_duplicates(subset = ["Name"], keep="first") keep the first and deletes the later ones (so no = 3 deleted)
# if we do df.drop_duplicates(subset = ["Name"], keep="last") keep the last and delete the previous ones (so no = 1 deleted)
